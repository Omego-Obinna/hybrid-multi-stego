...
# -*- coding: utf-8 -*-
"""
WOW baseline + pySTC embedding for BOSSBase.
...
Pipeline per cover image and payload bpp:
  ...
  3) rho <- WOW adaptive distortion cost map
  4) stego <- pySTC.hide(m, cover, rho, rho, seed, ...)
  5) verify extraction with pySTC.unhide(...), then log metrics.
...
Outputs:
  out-root/
    ...
    wow_0.200/*.pgm
    wow_0.400/*.pgm
    logs/embed_log_wow.csv
    logs/embed_log_wow.jsonl
    logs/embed_summary_wow.csv
"""
...
def wow_cost_map(img_u8: np.ndarray, wet_cost: float = 1e10, p: float = -1.0, eps: float = 1e-10) -> np.ndarray:
    """
    Canonical WOW-style spatial distortion cost map.

    This follows the structure of the public WOW implementation pattern:
    Daubechies-8 directional filters, residual suitability maps, and reciprocal
    Holder aggregation. The embedding itself is performed by pySTC or the proxy
    backend, not by the old probability-based simulator.
    """
    if convolve2d is None:
        # Conservative fallback: keep script runnable, but log algo_params will
        # state that scipy was unavailable. Install scipy for final WOW runs.
        var = local_variance_map(img_u8)
        rho = 1.0 / (eps + var.astype(np.float64))
        finite = rho[np.isfinite(rho)]
        hi = np.percentile(finite, 99.9) if finite.size else wet_cost
        return np.clip(np.nan_to_num(rho, nan=hi, posinf=hi, neginf=hi), 0, min(hi, wet_cost)).astype(np.float32)

    cover = img_u8.astype(np.float64)

    # Daubechies-8 high-pass decomposition filter used in the public WOW code.
    hpdf = np.array([
    ...
    lpdf = ((-1) ** np.arange(hpdf.shape[1]) * hpdf[:, ::-1]).astype(np.float64)
    ...
    pad = max(f.shape[0] for f in filters)
    cover_padded = np.pad(cover, ((pad, pad), (pad, pad)), mode="symmetric")
    xi = []
    for F in filters:
        R = convolve2d(cover_padded, F, mode="same", boundary="symm")
        suitability = convolve2d(1.0 / (np.abs(R) + eps), np.abs(np.rot90(F, 2)), mode="same", boundary="symm")
        xi.append(suitability[pad:pad + cover.shape[0], pad:pad + cover.shape[1]])

    rho = (np.power(xi[0], p) + np.power(xi[1], p) + np.power(xi[2], p)) ** (-1.0 / p)
    rho = np.nan_to_num(rho, nan=wet_cost, posinf=wet_cost, neginf=wet_cost)
    rho = np.minimum(rho, wet_cost)

    # Wet costs at boundaries, matching the public WOW convention.
    rho_p1 = rho.copy()
    rho_m1 = rho.copy()
    rho_p1[img_u8 >= 255] = wet_cost
    rho_m1[img_u8 <= 0] = wet_cost

    # pySTC accepts one cost map for +1 and one for -1; the wrapper used here
    # passes a symmetric map. To respect boundary wet costs in a single map,
    # take the safer of the two directional costs.
    rho_sym = np.maximum(rho_p1, rho_m1)
    finite = rho_sym[np.isfinite(rho_sym) & (rho_sym < wet_cost)]
    if finite.size:
        lo, hi = np.percentile(finite, [0.1, 99.9])
        rho_sym = np.clip(rho_sym, lo, min(hi, wet_cost))
    return rho_sym.astype(np.float32)
...
def run_pystc_subprocess(
    ...
    msg_bytes, padded = bits_to_bytes(payload_bits)
    with tempfile.TemporaryDirectory(prefix="wow_pystc_") as td:
        td_path = Path(td)
...
def iter_progress(items: List[Tuple[Path, float]], progress: str):
    if tqdm is not None and progress in {"auto", "bar"}:
        yield from tqdm(items, desc="WOW embedding", unit="job")
    else:
        total = len(items)
        step = max(1, total // 20)
        for i, item in enumerate(items, 1):
            if progress != "none" and (i == 1 or i % step == 0 or i == total):
                print(f"[WOW embedding] {100*i/total:5.1f}% ({i}/{total})", flush=True)
            yield item
...
def process_one(
    ...
    method = "WOW"
    out_dir = Path(args.out_root) / f"wow_{payload_bpp:.3f}"
    ...
    try:
        ...
        b_bits = hmac_bits(payload_key, b"wow-payload", L) if args.deterministic_payload else np.random.default_rng(args.seed).integers(0, 2, L, dtype=np.uint8)

        rho = wow_cost_map(cover, wet_cost=args.wet_cost, p=args.holder_p, eps=args.eps)
        ...
        row.update({
            ...
            **gpu,
            ...
            "algo_params": json.dumps({
                ...
                "cost_map": "WOW directional filters; db8 residual suitability; reciprocal Holder aggregation",
                "holder_p": args.holder_p,
    ...
    append_csv(logs_dir / "embed_log_wow.csv", CSV_HEADER, row)
    append_jsonl(logs_dir / "embed_log_wow.jsonl", row)
    return row
...
def summarize(rows: List[Dict[str, object]], out_root: Path, run_id: str) -> None:
    ...
    for payload, rs in sorted(by_payload.items()):
        ...
        row = {
            ...
            "out_dir": str(out_root / f"wow_{payload:.3f}"),
        }
        append_csv(logs / "embed_summary_wow.csv", SUMMARY_HEADER, row)


# ------------------------------- CLI -------------------------------

def parse_args():
    p = argparse.ArgumentParser(description="WOW baseline + pySTC adaptive-cost embedding for BOSSBase.")
    ...
    p.add_argument("--eps", type=float, default=1e-10, help="Stabiliser used in the WOW residual suitability computation")
    p.add_argument("--holder-p", type=float, default=-1.0, help="Holder norm parameter used by WOW")
...
def main():
    ...
    run_id = args.run_id or f"wow-{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S')}-{uuid.uuid4().hex[:6]}"
    ...
    print(f"[OK] Completed WOW run_id={run_id}")
    print(f"Logs:\n  - {logs_dir/'embed_log_wow.csv'}\n  - {logs_dir/'embed_log_wow.jsonl'}\n  - {logs_dir/'embed_summary_wow.csv'}")
