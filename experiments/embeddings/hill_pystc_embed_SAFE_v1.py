...
# -*- coding: utf-8 -*-
"""
HILL baseline + pySTC embedding for BOSSBase.

This is a conventional cover-modification (CMO) baseline script. It does
not implement the hybrid XOR masking rule
    b = m XOR gamma1 XOR gamma2 XOR k_stego,
and it does not implement the Ours-QIM cover-conditioned dither mask or
QIM cost modulation. The embedded payload is a direct pseudorandom message
...
HILL cost-map pipeline per cover image:
  1) High-pass residual using the KB/HILL kernel
         [[-1, 2, -1], [2, -4, 2], [-1, 2, -1]]
  2) Local residual aggregation with a 3x3 averaging low-pass filter L1
  3) Reciprocal suitability conversion followed by a 15x15 averaging
     low-pass filter L2 (cost spreading)
  4) pySTC embedding under the resulting additive cost map.

This follows the HILL filter chain described by Li et al. and illustrated in
the pySTC README. The gamma1/gamma2 columns are retained only for CSV-schema
compatibility with the hybrid methods; they are not used in HILL payload
construction or embedding.
...
Outputs:
  out-root/
    ...
    hill_0.200/*.pgm
    hill_0.400/*.pgm
    logs/embed_log_hill.csv
    logs/embed_log_hill.jsonl
    logs/embed_summary_hill.csv
"""
...
def hill_cost_map(img_u8: np.ndarray, wet_cost: float = 1e10, sigma: float = 1e-10) -> np.ndarray:
    """
    Paper-aligned HILL (HIgh-pass, Low-pass, Low-pass) distortion map.

    The filter chain is the HILL formulation used in the pySTC example and
    described in the original HILL paper:
        residual = H * X
        suitability = L1 * |residual|
        rho = L2 * (1 / (suitability + sigma))
    where H is the KB/HILL high-pass kernel, L1 is a 3x3 mean filter and
    L2 is a 15x15 mean filter. Smaller rho identifies safer embedding sites.

    A small sigma prevents singular costs in perfectly flat regions. The
    caller passes the resulting additive costs to pySTC; no probability or
    Lagrangian embedding simulator is used for final baseline runs.
    """
    if convolve2d is None:
        raise RuntimeError("scipy is required for the paper-aligned HILL cost map; install scipy")

    cover = img_u8.astype(np.float64)
    H = np.array([
        [-1.0,  2.0, -1.0],
        [ 2.0, -4.0,  2.0],
        [-1.0,  2.0, -1.0],
    ], dtype=np.float64)
    L1 = np.ones((3, 3), dtype=np.float64) / 9.0
    L2 = np.ones((15, 15), dtype=np.float64) / 225.0

    residual = convolve2d(cover, H, mode="same", boundary="symm")
    suitability = convolve2d(np.abs(residual), L1, mode="same", boundary="symm")
    rho = convolve2d(1.0 / (suitability + float(sigma)), L2, mode="same", boundary="symm")
    rho = np.nan_to_num(rho, nan=wet_cost, posinf=wet_cost, neginf=wet_cost)
    rho = np.minimum(rho, float(wet_cost))

    # Keep boundary operations safe in the symmetric-cost backend used across
    # all evaluated baselines: boundary pixels are treated as wet locations.
    rho[img_u8 <= 0] = float(wet_cost)
    rho[img_u8 >= 255] = float(wet_cost)
...
def run_pystc_subprocess(
    ...
    msg_bytes, padded = bits_to_bytes(payload_bits)
    with tempfile.TemporaryDirectory(prefix="hill_pystc_") as td:
        td_path = Path(td)
...
def iter_progress(items: List[Tuple[Path, float]], progress: str):
    if tqdm is not None and progress in {"auto", "bar"}:
        yield from tqdm(items, desc="HILL embedding", unit="job")
    else:
        total = len(items)
        step = max(1, total // 20)
        for i, item in enumerate(items, 1):
            if progress != "none" and (i == 1 or i % step == 0 or i == total):
                print(f"[HILL embedding] {100*i/total:5.1f}% ({i}/{total})", flush=True)
            yield item
...
def process_one(
    ...
    method = "HILL"
    out_dir = Path(args.out_root) / f"hill_{payload_bpp:.3f}"
    ...
    try:
        ...
        b_bits = hmac_bits(payload_key, b"hill-payload", L) if args.deterministic_payload else np.random.default_rng(args.seed).integers(0, 2, L, dtype=np.uint8)

        rho = hill_cost_map(cover, wet_cost=args.wet_cost, sigma=args.hill_sigma)
        ...
        row.update({
            ...
            **gpu,
            ...
            "algo_params": json.dumps({
                ...
                "cost_map": "HILL KB high-pass; 3x3 low-pass; reciprocal suitability; 15x15 low-pass cost spreading",
                "hill_sigma": args.hill_sigma,
                "wet_cost": args.wet_cost,
                "boundary_policy": "boundary pixels treated as wet in symmetric pySTC map",
    ...
    append_csv(logs_dir / "embed_log_hill.csv", CSV_HEADER, row)
    append_jsonl(logs_dir / "embed_log_hill.jsonl", row)
    return row
...
def summarize(rows: List[Dict[str, object]], out_root: Path, run_id: str) -> None:
    ...
    for payload, rs in sorted(by_payload.items()):
        ...
        row = {
            ...
            "out_dir": str(out_root / f"hill_{payload:.3f}"),
        }
        append_csv(logs / "embed_summary_hill.csv", SUMMARY_HEADER, row)


# ------------------------------- CLI -------------------------------

def parse_args():
    p = argparse.ArgumentParser(description="HILL baseline + pySTC adaptive-cost embedding for BOSSBase.")
    ...
    p.add_argument("--hill-sigma", type=float, default=1e-10, help="Stabiliser for reciprocal HILL suitability in flat regions")
    ...
    p.add_argument("--stc-backend", choices=["auto", "pystc", "proxy"], default="pystc",
                   help="Embedding backend: pystc for final results; auto may fall back to proxy and must not be merged with STC-only results.")
...
def main():
    ...
    run_id = args.run_id or f"hill-{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S')}-{uuid.uuid4().hex[:6]}"
    ...
    print(f"[OK] Completed HILL run_id={run_id}")
    print(f"Logs:\n  - {logs_dir/'embed_log_hill.csv'}\n  - {logs_dir/'embed_log_hill.jsonl'}\n  - {logs_dir/'embed_summary_hill.csv'}")
