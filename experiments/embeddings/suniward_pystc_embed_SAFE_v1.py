...
# -*- coding: utf-8 -*-
"""
S-UNIWARD baseline + pySTC embedding for BOSSBase.
...
Pipeline per cover image and payload bpp:
  ...
  3) rho <- S-UNIWARD adaptive distortion cost map
...
Outputs:
  out-root/
    ...
    suniward_0.200/*.pgm
    suniward_0.400/*.pgm
    logs/embed_log_suniward.csv
    logs/embed_log_suniward.jsonl
    logs/embed_summary_suniward.csv
"""
...
def suniward_cost_map(img_u8: np.ndarray, wet_cost: float = 1e10, sigma: float = 1.0) -> np.ndarray:
    """
    Spatial S-UNIWARD-style additive distortion cost map.

    This follows the public S-UNIWARD/Wavelet Universal Distortion structure:
    Daubechies-8 directional wavelet filters (LH, HL, HH), residual suitability
    maps 1/(|R_k| + sigma), and additive aggregation of the impact of changing
    a pixel through the rotated absolute filters. Embedding is performed by
    pySTC rather than by the old Lagrangian/probability simulator.
    """
    if convolve2d is None:
        var = local_variance_map(img_u8).astype(np.float64)
        rho = 1.0 / (1e-12 + var)
    ...
    # Daubechies-8 high-pass decomposition filter used by UNIWARD-family costs.
    hpdf = np.array([
    ...
    rho = np.zeros_like(cover, dtype=np.float64)

    for F in filters:
        R = convolve2d(cover_padded, F, mode="same", boundary="symm")
        xi = convolve2d(1.0 / (np.abs(R) + float(sigma)), np.abs(np.rot90(F, 2)), mode="same", boundary="symm")
        rho += xi[pad:pad + cover.shape[0], pad:pad + cover.shape[1]]

    rho = np.nan_to_num(rho, nan=wet_cost, posinf=wet_cost, neginf=wet_cost)
    rho = np.minimum(rho, wet_cost)

    # Wet costs at boundaries. Since pySTC is called with symmetric +1/-1 costs,
    # use the safer boundary-aware map.
    rho_p1 = rho.copy()
    rho_m1 = rho.copy()
...
def run_pystc_subprocess(
    ...
    msg_bytes, padded = bits_to_bytes(payload_bits)
    with tempfile.TemporaryDirectory(prefix="suniward_pystc_") as td:
        td_path = Path(td)
...
def iter_progress(items: List[Tuple[Path, float]], progress: str):
    if tqdm is not None and progress in {"auto", "bar"}:
        yield from tqdm(items, desc="S-UNIWARD embedding", unit="job")
    else:
        total = len(items)
        step = max(1, total // 20)
        for i, item in enumerate(items, 1):
            if progress != "none" and (i == 1 or i % step == 0 or i == total):
                print(f"[S-UNIWARD embedding] {100*i/total:5.1f}% ({i}/{total})", flush=True)
            yield item
...
def process_one(
    ...
    method = "S-UNIWARD"
    out_dir = Path(args.out_root) / f"suniward_{payload_bpp:.3f}"
    ...
    try:
        ...
        b_bits = hmac_bits(payload_key, b"suniward-payload", L) if args.deterministic_payload else np.random.default_rng(args.seed).integers(0, 2, L, dtype=np.uint8)

        rho = suniward_cost_map(cover, wet_cost=args.wet_cost, sigma=args.sigma)
        ...
        row.update({
            ...
            **gpu,
            ...
            "algo_params": json.dumps({
                ...
                "cost_map": "S-UNIWARD spatial directional wavelet distortion; db8 residual suitability; additive aggregation",
    ...
    append_csv(logs_dir / "embed_log_suniward.csv", CSV_HEADER, row)
    append_jsonl(logs_dir / "embed_log_suniward.jsonl", row)
    return row
...
def summarize(rows: List[Dict[str, object]], out_root: Path, run_id: str) -> None:
    ...
    for payload, rs in sorted(by_payload.items()):
        ...
        row = {
            ...
            "out_dir": str(out_root / f"suniward_{payload:.3f}"),
        }
        append_csv(logs / "embed_summary_suniward.csv", SUMMARY_HEADER, row)


# ------------------------------- CLI -------------------------------

def parse_args():
    p = argparse.ArgumentParser(description="S-UNIWARD baseline + pySTC adaptive-cost embedding for BOSSBase.")
    ...
    p.add_argument("--sigma", type=float, default=1.0, help="S-UNIWARD stabiliser sigma used in 1/(|R|+sigma)")
...
def main():
    ...
    run_id = args.run_id or f"suniward-{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S')}-{uuid.uuid4().hex[:6]}"
    ...
    print(f"[OK] Completed S-UNIWARD run_id={run_id}")
    print(f"Logs:\n  - {logs_dir/'embed_log_suniward.csv'}\n  - {logs_dir/'embed_log_suniward.jsonl'}\n  - {logs_dir/'embed_summary_suniward.csv'}")
