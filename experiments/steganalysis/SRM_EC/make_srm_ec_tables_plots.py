...
def plot_score_grouped_bars(df: pd.DataFrame, fig_dir: Path) -> None:
    ...
    The upper y-axis limit is computed from the observed maximum so that
    high-payload bars and their numeric labels are never clipped.
    """
    x, x_labels = payload_axis(df)
    ...
    # Compute a data-driven upper limit before drawing labels.  The 0.08
    # headroom keeps labels visible above the tallest bar (e.g., AUC=0.886).
    all_values = np.concatenate(
        [df[metric].astype(float).to_numpy() for metric in metrics]
    )
    max_score = float(np.nanmax(all_values))
    y_lower = 0.45
    y_upper = min(1.00, max(0.80, max_score + 0.08))
